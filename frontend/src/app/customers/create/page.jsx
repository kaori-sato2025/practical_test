...
export default function CreatePage() {
  ...
  const [errorMessage, setErrorMessage] = useState(""); //エラーメッセージ用ステート追加

  const handleSubmit = async (event) => {
    event.preventDefault();
    const formData = new FormData(formRef.current);
    const customerId = formData.get("customer_id"); //customer_idを取得
    if (!customerId || customerId.trim() === "") {
      setErrorMessage("customer_idは必ず入力してください");
      return;
    } //customer_idが空欄の時にエラーメッセージを表示
